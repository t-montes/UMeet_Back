...
export class SettingsService {
    ...
    async findOne(id: string): Promise<SettingsEntity> {
        console.log("findOne - buscando settings con id:", id);
        const settings: SettingsEntity = await this.settingsRepository.findOne({
            where: { id },
            relations: ['user'],
        });
        console.log("findOne - settings encontrado:", settings);
        if (!settings)
            throw new BadRequestException('The settings with the given id was not found');
        return settings;
    }
    ...
    async update(id: string, settings: SettingsEntity): Promise<SettingsEntity> {
        const persistedSettings: SettingsEntity = await this.settingsRepository.findOne({
            where: { id },
        });
        if (!persistedSettings)
            throw new BadRequestException('The settings with the given id was not found');

        return await this.settingsRepository.save({ ...persistedSettings, ...settings });
    }
